...
public class CombatManager {
    ...
    public float calculateDamage(Card card1, Card card2){
        if (card1.getMonstertype() == Monstertype.magicdice){
            Random rand = new Random();
            if (rand.nextInt(6) > 3){
                return 999;
            }
        }
        if (card1.getMonstertype() != Monstertype.Spell){
            if (card2.getMonstertype() != Monstertype.Spell) {
                ...
                if (card1.getMonstertype() == Monstertype.Knight && card2.getElementtype() == Elementtype.Water){
                    return -1;
                }
            }
        }
        if (card2.getMonstertype() == Monstertype.Kraken){
            return 0;
        }
